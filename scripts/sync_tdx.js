const fs = require('fs');
const path = require('path');
const fetch = globalThis.fetch || (() => {
  try { return require('node-fetch'); } catch (e) { throw new Error("環境中缺少 fetch API，請使用 Node.js 18+ 或安裝 node-fetch"); }
})();

// 經典備份全台充電站資料庫 (當未提供 TDX 憑證或 API 異常時之保底 fallback)
const DEFAULT_STATIONS = [
  {
    "id": "dc-001",
    "name": "關西服務區快充站 (國道3號)",
    "cat": "DC",
    "category": "DC",
    "lat": 24.7936,
    "lng": 121.1685,
    "op": "EVOASIS / U-POWER",
    "operator": "EVOASIS / U-POWER",
    "type": "CCS1 / CCS2 / TPC",
    "power": "360 kW (DC超快充)",
    "addr": "新竹縣關西鎮新興路63號",
    "address": "新竹縣關西鎮新興路63號",
    "price": "約 $8.5 - $11.5 /度"
  },
  {
    "id": "dc-002",
    "name": "泰安服務區(北上)快充站 (國道1號)",
    "cat": "DC",
    "category": "DC",
    "lat": 24.3315,
    "lng": 120.7108,
    "op": "華城 EVALUE",
    "operator": "華城 EVALUE",
    "type": "CCS1 / CCS2",
    "power": "200 kW (DC快充)",
    "addr": "台中市后里區安眉路113號",
    "address": "台中市后里區安眉路113號",
    "price": "約 $9.0 - $11.0 /度"
  },
  {
    "id": "dc-003",
    "name": "清水服務區快充站 (國道3號)",
    "cat": "DC",
    "category": "DC",
    "lat": 24.2705,
    "lng": 120.5901,
    "op": "iCharging / U-POWER",
    "operator": "iCharging / U-POWER",
    "type": "CCS1 / CCS2",
    "power": "360 kW (DC超快充)",
    "addr": "台中市清水區吳厝里七項巷41-4號",
    "address": "台中市清水區吳厝里七項巷41-4號",
    "price": "約 $9.5 - $12.0 /度"
  },
  {
    "id": "dc-004",
    "name": "西螺服務區(南下)快充站 (國道1號)",
    "cat": "DC",
    "category": "DC",
    "lat": 23.7997,
    "lng": 120.4636,
    "op": "U-POWER",
    "operator": "U-POWER",
    "type": "CCS1 / TPC / CCS2",
    "power": "360 kW (DC超快充)",
    "addr": "雲林縣西螺鎮振興里270號",
    "address": "雲林縣西螺鎮振興里270號",
    "price": "約 $9.5 - $11.5 /度"
  },
  {
    "id": "dc-005",
    "name": "南投服務區快充站 (國道3號)",
    "cat": "DC",
    "category": "DC",
    "lat": 23.9012,
    "lng": 120.7225,
    "op": "華城 EVALUE",
    "operator": "華城 EVALUE",
    "type": "CCS1 / CCS2",
    "power": "200 kW (DC快充)",
    "addr": "南投縣南投市布園路300號",
    "address": "南投縣南投市布園路300號",
    "price": "約 $9.0 - $11.0 /度"
  },
  {
    "id": "dc-006",
    "name": "東山服務區快充站 (國道3號)",
    "cat": "DC",
    "category": "DC",
    "lat": 23.2758,
    "lng": 120.4132,
    "op": "EVOASIS",
    "operator": "EVOASIS",
    "type": "CCS1 / CCS2",
    "power": "360 kW (DC超快充)",
    "addr": "台南市東山區枋嶋路412號",
    "address": "台南市東山區枋嶋路412號",
    "price": "約 $8.5 - $11.0 /度"
  },
  {
    "id": "dc-007",
    "name": "仁德服務區(南下)快充站 (國道1號)",
    "cat": "DC",
    "category": "DC",
    "lat": 22.9568,
    "lng": 120.2521,
    "op": "華城 EVALUE",
    "operator": "華城 EVALUE",
    "type": "CCS1 / CCS2",
    "power": "360 kW (DC超快充)",
    "addr": "台南市仁德區中正路二段1255號",
    "address": "台南市仁德區中正路二段1255號",
    "price": "約 $9.0 - $11.5 /度"
  },
  {
    "id": "dc-008",
    "name": "蘇澳服務區快充站 (國道5號)",
    "cat": "DC",
    "category": "DC",
    "lat": 24.6068,
    "lng": 121.8315,
    "op": "U-POWER",
    "operator": "U-POWER",
    "type": "CCS1 / CCS2 / TPC",
    "power": "360 kW (DC超快充)",
    "addr": "宜蘭縣蘇澳鎮新馬路6號",
    "address": "宜蘭縣蘇澳鎮新馬路6號",
    "price": "約 $9.5 - $11.5 /度"
  },
  {
    "id": "dc-009",
    "name": "U-POWER 台北內湖超級快充站",
    "cat": "DC",
    "category": "DC",
    "lat": 25.0621,
    "lng": 121.5742,
    "op": "U-POWER",
    "operator": "U-POWER",
    "type": "CCS1 / CCS2",
    "power": "360 kW (DC超快充)",
    "addr": "台北市內湖區行忠路2號",
    "address": "台北市內湖區行忠路2號",
    "price": "約 $9.5 - $11.0 /度"
  },
  {
    "id": "dc-010",
    "name": "EVOASIS 台北三創快充站",
    "cat": "DC",
    "category": "DC",
    "lat": 25.0452,
    "lng": 121.5312,
    "op": "EVOASIS",
    "operator": "EVOASIS",
    "type": "CCS1 / CCS2",
    "power": "180 kW (DC快充)",
    "addr": "台北市中正區市民大道三段2號B3",
    "address": "台北市中正區市民大道三段2號B3",
    "price": "約 $8.5 - $10.5 /度"
  },
  {
    "id": "dc-011",
    "name": "華城 EVALUE 新莊超級快充站",
    "cat": "DC",
    "category": "DC",
    "lat": 25.0489,
    "lng": 121.4512,
    "op": "華城 EVALUE",
    "operator": "華城 EVALUE",
    "type": "CCS1 / CCS2",
    "power": "240 kW (DC快充)",
    "addr": "新北市新莊區中正路655號",
    "address": "新北市新莊區中正路655號",
    "price": "約 $9.0 - $11.0 /度"
  },
  {
    "id": "dc-012",
    "name": "U-POWER 桃園經國超級快充站",
    "cat": "DC",
    "category": "DC",
    "lat": 25.0185,
    "lng": 121.3021,
    "op": "U-POWER",
    "operator": "U-POWER",
    "type": "CCS1 / CCS2",
    "power": "360 kW (DC超快充)",
    "addr": "桃園市桃園區經國路888號",
    "address": "桃園市桃園區經國路888號",
    "price": "約 $9.5 - $11.0 /度"
  },
  {
    "id": "dc-013",
    "name": "EVOASIS 桃園青埔高鐵快充站",
    "cat": "DC",
    "category": "DC",
    "lat": 25.0135,
    "lng": 121.2145,
    "op": "EVOASIS",
    "operator": "EVOASIS",
    "type": "CCS1 / CCS2",
    "power": "360 kW (DC超快充)",
    "addr": "桃園市中壢區高鐵站前西路一段",
    "address": "桃園市中壢區高鐵站前西路一段",
    "price": "約 $8.8 - $10.8 /度"
  },
  {
    "id": "dc-014",
    "name": "U-POWER 新竹竹北超級快充站",
    "cat": "DC",
    "category": "DC",
    "lat": 24.8212,
    "lng": 121.0185,
    "op": "U-POWER",
    "operator": "U-POWER",
    "type": "CCS1 / CCS2",
    "power": "360 kW (DC超快充)",
    "addr": "新竹縣竹北市光明六路東二段",
    "address": "新竹縣竹北市光明六路東二段",
    "price": "約 $9.5 - $11.0 /度"
  },
  {
    "id": "dc-015",
    "name": "EVOASIS 新竹公道五快充站",
    "cat": "DC",
    "category": "DC",
    "lat": 24.8054,
    "lng": 120.9932,
    "op": "EVOASIS",
    "operator": "EVOASIS",
    "type": "CCS1 / CCS2",
    "power": "180 kW (DC快充)",
    "addr": "新竹市東區公道五路二段",
    "address": "新竹市東區公道五路二段",
    "price": "約 $8.5 - $10.5 /度"
  },
  {
    "id": "dc-016",
    "name": "U-POWER 台中五權西超級快充站",
    "cat": "DC",
    "category": "DC",
    "lat": 24.1485,
    "lng": 120.6312,
    "op": "U-POWER",
    "operator": "U-POWER",
    "type": "CCS1 / CCS2",
    "power": "360 kW (DC超快充)",
    "addr": "台中市南屯區五權西路二段",
    "address": "台中市南屯區五權西路二段",
    "price": "約 $9.5 - $11.0 /度"
  },
  {
    "id": "dc-017",
    "name": "EVOASIS 台中中清快充站",
    "cat": "DC",
    "category": "DC",
    "lat": 24.1854,
    "lng": 120.6542,
    "op": "EVOASIS",
    "operator": "EVOASIS",
    "type": "CCS1 / CCS2",
    "power": "360 kW (DC超快充)",
    "addr": "台中市北屯區中清路二段",
    "address": "台中市北屯區中清路二段",
    "price": "約 $8.8 - $10.8 /度"
  },
  {
    "id": "dc-018",
    "name": "EVOASIS 台南安平快充站",
    "cat": "DC",
    "category": "DC",
    "lat": 22.9912,
    "lng": 120.1654,
    "op": "EVOASIS",
    "operator": "EVOASIS",
    "type": "CCS1 / CCS2",
    "power": "180 kW (DC快充)",
    "addr": "台南市安平區安平路",
    "address": "台南市安平區安平路",
    "price": "約 $8.5 - $10.5 /度"
  },
  {
    "id": "dc-019",
    "name": "U-POWER 台南和緯超級快充站",
    "cat": "DC",
    "category": "DC",
    "lat": 23.0045,
    "lng": 120.1985,
    "op": "U-POWER",
    "operator": "U-POWER",
    "type": "CCS1 / CCS2",
    "power": "360 kW (DC超快充)",
    "addr": "台南市北區和緯路四段",
    "address": "台南市北區和緯路四段",
    "price": "約 $9.5 - $11.0 /度"
  },
  {
    "id": "dc-020",
    "name": "U-POWER 高雄左營超級快充站",
    "cat": "DC",
    "category": "DC",
    "lat": 22.6885,
    "lng": 120.3112,
    "op": "U-POWER",
    "operator": "U-POWER",
    "type": "CCS1 / CCS2",
    "power": "360 kW (DC超快充)",
    "addr": "高雄市左營區文府路",
    "address": "高雄市左營區文府路",
    "price": "約 $9.5 - $11.0 /度"
  },
  {
    "id": "dc-021",
    "name": "EVOASIS 高雄三多快充站",
    "cat": "DC",
    "category": "DC",
    "lat": 22.6185,
    "lng": 120.3085,
    "op": "EVOASIS",
    "operator": "EVOASIS",
    "type": "CCS1 / CCS2",
    "power": "180 kW (DC快充)",
    "addr": "高雄市苓雅區三多三路",
    "address": "高雄市苓雅區三多三路",
    "price": "約 $8.5 - $10.5 /度"
  },
  {
    "id": "dc-022",
    "name": "EVOASIS 花蓮火車站前快充站",
    "cat": "DC",
    "category": "DC",
    "lat": 23.9933,
    "lng": 121.6015,
    "op": "EVOASIS",
    "operator": "EVOASIS",
    "type": "CCS1 / CCS2",
    "power": "180 kW (DC快充)",
    "addr": "花蓮縣花蓮市國聯一路100號",
    "address": "花蓮縣花蓮市國聯一路100號",
    "price": "約 $9.0 - $11.0 /度"
  },
  {
    "id": "dc-023",
    "name": "U-POWER 台東火車站前超級快充站",
    "cat": "DC",
    "category": "DC",
    "lat": 22.7935,
    "lng": 121.1232,
    "op": "U-POWER",
    "operator": "U-POWER",
    "type": "CCS1 / CCS2",
    "power": "360 kW (DC超快充)",
    "addr": "台東縣台東市岩灣路101巷",
    "address": "台東縣台東市岩灣路101巷",
    "price": "約 $9.5 - $11.5 /度"
  },
  {
    "id": "ac-001",
    "name": "台北市政府地下停車場",
    "cat": "AC",
    "category": "AC",
    "lat": 25.0375,
    "lng": 121.5644,
    "op": "台北市交通局",
    "operator": "台北市交通局",
    "type": "J1772 / Type 2",
    "power": "7 kW (AC慢充)",
    "addr": "台北市信義區市府路1號",
    "address": "台北市信義區市府路1號",
    "price": "依停車場費率"
  },
  {
    "id": "ac-002",
    "name": "台北松山車站地下停車場",
    "cat": "AC",
    "category": "AC",
    "lat": 25.0489,
    "lng": 121.5785,
    "op": "台北市交通局",
    "operator": "台北市交通局",
    "type": "J1772",
    "power": "7 kW (AC慢充)",
    "addr": "台北市信義區松山路11號",
    "address": "台北市信義區松山路11號",
    "price": "依停車場費率"
  },
  {
    "id": "ac-003",
    "name": "台北大安森林公園地下停車場",
    "cat": "AC",
    "category": "AC",
    "lat": 25.0298,
    "lng": 121.5358,
    "op": "台北市交通局",
    "operator": "台北市交通局",
    "type": "J1772",
    "power": "7 kW (AC慢充)",
    "addr": "台北市大安區建國南路二段2號",
    "address": "台北市大安區建國南路二段2號",
    "price": "依停車場費率"
  },
  {
    "id": "ac-004",
    "name": "板橋車站地下停車場",
    "cat": "AC",
    "category": "AC",
    "lat": 25.0135,
    "lng": 121.4632,
    "op": "新北市交通局",
    "operator": "新北市交通局",
    "type": "J1772",
    "power": "7 kW (AC慢充)",
    "addr": "新北市板橋區縣民大道二段7號",
    "address": "新北市板橋區縣民大道二段7號",
    "price": "依停車場費率"
  },
  {
    "id": "ac-005",
    "name": "新莊運動公園地下停車場",
    "cat": "AC",
    "category": "AC",
    "lat": 25.0412,
    "lng": 121.4518,
    "op": "新北市交通局",
    "operator": "新北市交通局",
    "type": "J1772",
    "power": "7 kW (AC慢充)",
    "addr": "新北市新莊區和興街66號",
    "address": "新北市新莊區和興街66號",
    "price": "依停車場費率"
  },
  {
    "id": "ac-006",
    "name": "桃園藝文特區地下停車場",
    "cat": "AC",
    "category": "AC",
    "lat": 25.0045,
    "lng": 121.3012,
    "op": "桃園市交通局",
    "operator": "桃園市交通局",
    "type": "J1772",
    "power": "7 kW (AC慢充)",
    "addr": "桃園市桃園區中正路1188號",
    "address": "桃園市桃園區中正路1188號",
    "price": "依停車場費率"
  },
  {
    "id": "ac-007",
    "name": "桃園中壢藝術館停車場",
    "cat": "AC",
    "category": "AC",
    "lat": 24.9585,
    "lng": 121.2245,
    "op": "桃園市交通局",
    "operator": "桃園市交通局",
    "type": "J1772",
    "power": "7 kW (AC慢充)",
    "addr": "桃園市中壢區中美路16號",
    "address": "桃園市中壢區中美路16號",
    "price": "依停車場費率"
  },
  {
    "id": "ac-008",
    "name": "新竹縣政府地下停車場",
    "cat": "AC",
    "category": "AC",
    "lat": 24.8387,
    "lng": 121.0125,
    "op": "新竹縣政府",
    "operator": "新竹縣政府",
    "type": "J1772",
    "power": "7 kW (AC慢充)",
    "addr": "新竹縣竹北市光明六路10號",
    "address": "新竹縣竹北市光明六路10號",
    "price": "依停車場費率"
  },
  {
    "id": "ac-009",
    "name": "新竹巨城購物中心地下停車場",
    "cat": "AC",
    "category": "AC",
    "lat": 24.8095,
    "lng": 120.9745,
    "op": "遠東巨城",
    "operator": "遠東巨城",
    "type": "J1772",
    "power": "7 kW (AC慢充)",
    "addr": "新竹市東區中央路229號",
    "address": "新竹市東區中央路229號",
    "price": "購物折抵 / 停車費率"
  },
  {
    "id": "ac-010",
    "name": "台中秋紅谷地下停車場",
    "cat": "AC",
    "category": "AC",
    "lat": 24.1672,
    "lng": 120.6394,
    "op": "台中市交通局",
    "operator": "台中市交通局",
    "type": "J1772",
    "power": "7 kW (AC慢充)",
    "addr": "台中市西屯區朝富路30號",
    "address": "台中市西屯區朝富路30號",
    "price": "依停車場費率"
  },
  {
    "id": "ac-011",
    "name": "台中草悟道地下停車場",
    "cat": "AC",
    "category": "AC",
    "lat": 24.1512,
    "lng": 120.6632,
    "op": "台中市交通局",
    "operator": "台中市交通局",
    "type": "J1772",
    "power": "7 kW (AC慢充)",
    "addr": "台中市西區館前路",
    "address": "台中市西區館前路",
    "price": "依停車場費率"
  },
  {
    "id": "ac-012",
    "name": "台中市政大樓地下停車場",
    "cat": "AC",
    "category": "AC",
    "lat": 24.1632,
    "lng": 120.6478,
    "op": "台中市交通局",
    "operator": "台中市交通局",
    "type": "J1772",
    "power": "7 kW (AC慢充)",
    "addr": "台中市西屯區台灣大道三段99號",
    "address": "台中市西屯區台灣大道三段99號",
    "price": "依停車場費率"
  },
  {
    "id": "ac-013",
    "name": "台南赤崁樓地下停車場",
    "cat": "AC",
    "category": "AC",
    "lat": 22.9975,
    "lng": 120.2025,
    "op": "台南市交通局",
    "operator": "台南市交通局",
    "type": "J1772",
    "power": "7 kW (AC慢充)",
    "addr": "台南市中西區民族路二段212號",
    "address": "台南市中西區民族路二段212號",
    "price": "依停車場費率"
  },
  {
    "id": "ac-014",
    "name": "台南海安路地下停車場",
    "cat": "AC",
    "category": "AC",
    "lat": 22.9945,
    "lng": 120.1978,
    "op": "台南市交通局",
    "operator": "台南市交通局",
    "type": "J1772",
    "power": "7 kW (AC慢充)",
    "addr": "台南市中西區海安路一段",
    "address": "台南市中西區海安路一段",
    "price": "依停車場費率"
  },
  {
    "id": "ac-015",
    "name": "高雄巨蛋地下停車場",
    "cat": "AC",
    "category": "AC",
    "lat": 22.6695,
    "lng": 120.3022,
    "op": "高雄市交通局",
    "operator": "高雄市交通局",
    "type": "J1772",
    "power": "7 kW (AC慢充)",
    "addr": "高雄市左營區博愛二路757號",
    "address": "高雄市左營區博愛二路757號",
    "price": "依停車場費率"
  },
  {
    "id": "ac-016",
    "name": "高雄駁二藝術特區停車場",
    "cat": "AC",
    "category": "AC",
    "lat": 22.6205,
    "lng": 120.2812,
    "op": "高雄市文化局",
    "operator": "高雄市文化局",
    "type": "J1772",
    "power": "7 kW (AC慢充)",
    "addr": "高雄市鹽埕區大勇路1號",
    "address": "高雄市鹽埕區大勇路1號",
    "price": "依停車場費率"
  },
  {
    "id": "ac-017",
    "name": "高雄文化中心地下停車場",
    "cat": "AC",
    "category": "AC",
    "lat": 22.6245,
    "lng": 120.3185,
    "op": "高雄市文化局",
    "operator": "高雄市文化局",
    "type": "J1772",
    "power": "7 kW (AC慢充)",
    "addr": "高雄市苓雅區五福一路67號",
    "address": "高雄市苓雅區五福一路67號",
    "price": "依停車場費率"
  },
  {
    "id": "ht-001",
    "name": "礁溪老爺酒店充電站",
    "cat": "AC",
    "category": "AC",
    "lat": 24.8321,
    "lng": 121.7584,
    "op": "礁溪老爺酒店",
    "operator": "礁溪老爺酒店",
    "type": "Type 2 / J1772 / Tesla TPC",
    "power": "7-11 kW (AC慢充)",
    "addr": "宜蘭縣礁溪鄉大忠村五峰路69號",
    "address": "宜蘭縣礁溪鄉大忠村五峰路69號",
    "price": "房客專屬免費預約充電"
  },
  {
    "id": "ht-002",
    "name": "台北晶華酒店地下停車場",
    "cat": "AC",
    "category": "AC",
    "lat": 25.0538,
    "lng": 121.5244,
    "op": "台北晶華酒店",
    "operator": "台北晶華酒店",
    "type": "Type 2 / Tesla TPC",
    "power": "7-11 kW (AC慢充)",
    "addr": "台北市中山區中山北路二段39巷3號",
    "address": "台北市中山區中山北路二段39巷3號",
    "price": "房客/用餐依停車場折抵"
  },
  {
    "id": "ht-003",
    "name": "日月潭涵碧樓充電站",
    "cat": "AC",
    "category": "AC",
    "lat": 23.8645,
    "lng": 120.9123,
    "op": "涵碧樓大飯店",
    "operator": "涵碧樓大飯店",
    "type": "Tesla TPC / Type 2",
    "power": "7-11 kW (AC慢充)",
    "addr": "南投縣魚池鄉中興路142號",
    "address": "南投縣魚池鄉中興路142號",
    "price": "房客專屬免費預約"
  },
  {
    "id": "ht-004",
    "name": "花蓮遠雄悅來大飯店充電站",
    "cat": "AC",
    "category": "AC",
    "lat": 23.9015,
    "lng": 121.6025,
    "op": "遠雄悅來大飯店",
    "operator": "遠雄悅來大飯店",
    "type": "J1772 / Tesla TPC",
    "power": "7 kW (AC慢充)",
    "addr": "花蓮縣壽豐鄉鹽寮村山嶺18號",
    "address": "花蓮縣壽豐鄉鹽寮村山嶺18號",
    "price": "房客免費專屬車位"
  },
  {
    "id": "ht-005",
    "name": "高雄萬豪酒店地下停車場",
    "cat": "AC",
    "category": "AC",
    "lat": 22.6575,
    "lng": 120.3065,
    "op": "高雄萬豪酒店 / 特斯拉",
    "operator": "高雄萬豪酒店 / 特斯拉",
    "type": "Tesla TPC / Type 2 / CCS2",
    "power": "7-22 kW (AC慢充/快充)",
    "addr": "高雄市鼓山區龍德新路222號",
    "address": "高雄市鼓山區龍德新路222號",
    "price": "依飯店停車場規範"
  },
  {
    "id": "ht-006",
    "name": "墾丁夏都沙灘酒店充電站",
    "cat": "AC",
    "category": "AC",
    "lat": 21.9421,
    "lng": 120.7915,
    "op": "夏都沙灘酒店",
    "operator": "夏都沙灘酒店",
    "type": "J1772 / Tesla TPC",
    "power": "7 kW (AC慢充)",
    "addr": "屏東縣恆春鎮墾丁路451號",
    "address": "屏東縣恆春鎮墾丁路451號",
    "price": "房客免費預約充電"
  }
];

// 取得 TDX OAuth2 Access Token
async function getTdxAccessToken(clientId, clientSecret) {
  const tokenUrl = 'https://tdx.transportdata.tw/auth/realms/TDXConnect/protocol/openid-connect/token';
  const params = new URLSearchParams();
  params.append('grant_type', 'client_credentials');
  params.append('client_id', clientId);
  params.append('client_secret', clientSecret);

  const res = await fetch(tokenUrl, {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    body: params
  });

  if (!res.ok) {
    throw new Error(`TDX Token 驗證失敗: ${res.status} ${res.statusText}`);
  }

  const data = await res.json();
  return data.access_token;
}

// 遞迴分頁拉取 TDX 全台所有充電站點資料 (支援全區 API 與各縣市/國道子路由備援)
async function fetchAllTdxStations(token) {
  const allStations = [];
  const top = 1000;

  console.log('開始分頁擷取 TDX 全台充電站資料 ($top=1000)...');

  // 1. 嘗試直接拉取全台總端點
  try {
    let skip = 0;
    let hasMore = true;
    while (hasMore) {
      const apiUrl = `https://tdx.transportdata.tw/api/basic/v1/EV/Station?$format=JSON&$top=${top}&$skip=${skip}`;
      console.log(`正在請求 TDX 全台總端點: skip=${skip}, top=${top}...`);

      const res = await fetch(apiUrl, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json'
        }
      });

      if (!res.ok) {
        console.warn(`TDX 全台總端點回應 (HTTP ${res.status}): ${res.statusText}`);
        break;
      }

      const pageData = await res.json();
      if (!Array.isArray(pageData) || pageData.length === 0) break;

      allStations.push(...pageData);
      console.log(`成功獲取 ${pageData.length} 筆資料 (累計: ${allStations.length} 筆)`);

      if (pageData.length < top) {
        hasMore = false;
      } else {
        skip += top;
      }
    }
  } catch (err) {
    console.warn('全台總端點請求異常:', err.message);
  }

  // 2. 若全台總端點為 0 筆，嘗試拉取國道與各縣市子端點
  if (allStations.length === 0) {
    console.log('嘗試從 TDX 國道與全台 22 縣市子端點聚合資料...');
    const subEndpoints = [
      'Freeway', 'Taipei', 'NewTaipei', 'Taoyuan', 'Taichung', 'Tainan', 'Kaohsiung',
      'Keelung', 'Hsinchu', 'HsinchuCounty', 'MiaoliCounty', 'ChanghuaCounty',
      'NantouCounty', 'YunlinCounty', 'Chiayi', 'ChiayiCounty', 'PingtungCounty',
      'YilanCounty', 'HualienCounty', 'TaitungCounty', 'PenghuCounty', 'KinmenCounty'
    ];

    for (const sub of subEndpoints) {
      try {
        const subUrl = `https://tdx.transportdata.tw/api/basic/v1/EV/Station/${sub === 'Freeway' ? 'Freeway' : 'City/' + sub}?$format=JSON&$top=500`;
        const res = await fetch(subUrl, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Accept': 'application/json'
          }
        });

        if (res.ok) {
          const list = await res.json();
          if (Array.isArray(list) && list.length > 0) {
            allStations.push(...list);
            console.log(`  [${sub}] 成功取得 ${list.length} 筆資料`);
          }
        }
      } catch (e) {
        // 忽略個別縣市連線例外
      }
    }
  }

  return allStations;
}

// 資料清洗與格式正規化
function cleanAndNormalizeStations(rawDataList) {
  const result = [];
  const seenPositions = new Set();

  rawDataList.forEach((st, idx) => {
    // 1. 座標驗證 (限台灣本島及澎金馬有效經緯度)
    const lat = st.StationPosition?.PositionLat;
    const lng = st.StationPosition?.PositionLon;

    if (typeof lat !== 'number' || typeof lng !== 'number') return;
    if (lat < 21.5 || lat > 26.5 || lng < 119.0 || lng > 122.5) return;

    // 2. 避免完全重疊重複站點
    const posKey = `${lat.toFixed(5)},${lng.toFixed(5)}`;
    if (seenPositions.has(posKey)) return;
    seenPositions.add(posKey);

    // 3. 站點名稱與營運商解析
    const name = typeof st.StationName === 'object'
      ? (st.StationName.Zh_tw || st.StationName.En || '電動車充電站')
      : (st.StationName || '電動車充電站');

    const op = typeof st.OperatorName === 'object'
      ? (st.OperatorName.Zh_tw || st.OperatorName.En || '充電營運商')
      : (st.OperatorName || '充電營運商');

    // 4. 充電槍規格與快慢充判斷
    let isDC = false;
    let maxPower = 0;
    const plugTypesSet = new Set();

    if (Array.isArray(st.Chargers) && st.Chargers.length > 0) {
      st.Chargers.forEach(c => {
        const power = Number(c.Power) || 0;
        if (power > maxPower) maxPower = power;

        if (c.PlugType) plugTypesSet.add(c.PlugType);

        if (power >= 30 ||
            /DC|CCS|CHAdeMO|Supercharger|快充/i.test(c.ChargingLevel || '') ||
            /CCS1|CCS2|CHAdeMO|TPC|NACS/i.test(c.PlugType || '')) {
          isDC = true;
        }
      });
    }

    const plugTypes = plugTypesSet.size > 0
      ? Array.from(plugTypesSet).join(' / ')
      : (isDC ? 'CCS1 / CCS2' : 'J1772');

    const powerText = maxPower > 0
      ? `${maxPower} kW (${isDC ? 'DC超快充' : 'AC慢充'})`
      : (isDC ? 'DC快充' : 'AC慢充');

    const cat = isDC ? 'DC' : 'AC';
    const address = st.Address || st.Location || '全台站點';
    const price = st.FareDescription || '依現場公告或各營運商 APP 為準';

    result.push({
      id: st.StationID || `tdx-${idx}`,
      name,
      cat,
      category: cat,
      lat,
      lng,
      op,
      operator: op,
      type: plugTypes,
      power: powerText,
      addr: address,
      address,
      price
    });
  });

  return result;
}

async function main() {
  const clientId = process.env.TDX_CLIENT_ID;
  const clientSecret = process.env.TDX_CLIENT_SECRET;
  let finalStations = DEFAULT_STATIONS;

  if (clientId && clientSecret) {
    try {
      console.log('連線至 TDX 驗證 Token...');
      const token = await getTdxAccessToken(clientId, clientSecret);
      console.log('TDX Token 驗證成功！');

      const rawStations = await fetchAllTdxStations(token);
      if (rawStations && rawStations.length > 0) {
        const cleaned = cleanAndNormalizeStations(rawStations);
        if (cleaned.length > 0) {
          finalStations = cleaned;
          console.log(`成功清洗並產出 ${cleaned.length} 筆全台充電站 (DC快充: ${cleaned.filter(s => s.cat === 'DC').length} 處, AC慢充: ${cleaned.filter(s => s.cat === 'AC').length} 處)`);
        }
      }
    } catch (err) {
      console.error('TDX API 連線或同步異常，啟用備援數據庫:', err.message);
    }
  } else {
    console.log('未設定 TDX_CLIENT_ID / TDX_CLIENT_SECRET，使用 40+ 筆全台精選充電站資料。');
  }

  // 輸出至 github_service/data/stations.json
  const outPath = path.join(__dirname, '../data/stations.json');
  fs.mkdirSync(path.dirname(outPath), { recursive: true });
  fs.writeFileSync(outPath, JSON.stringify(finalStations, null, 2), 'utf-8');

  // 同步輸出至專案根目錄 ev_stations_taiwan.json (便於本地測試與靜態預覽)
  const rootOutPath = path.join(__dirname, '../../ev_stations_taiwan.json');
  try {
    fs.writeFileSync(rootOutPath, JSON.stringify(finalStations, null, 2), 'utf-8');
  } catch (e) {
    // ignore
  }

  console.log(`成功同步完成！共輸出 ${finalStations.length} 筆充電站資料至 ${outPath}`);
}

main();
